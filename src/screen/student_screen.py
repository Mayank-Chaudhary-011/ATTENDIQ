...
def student_dashboard():
    ...
    for i, sub_node in enumerate(subjects):
        ...
        def unenroll_button():
                if st.button("Unenroll from tihs course", type='tertiary', width='stretch',key=f"unenroll_{sid}", icon=':material/delete_forever:'):
    ...
    if photo_source:
        ...
        with st.spinner("AI is scanning..."):
            ...
            if num_faces == 0:
                ...
                if detected:
                    # ✅ FIXED: removed ()
                    student_id = next(iter(detected), None)
    ...
    if show_registration:
        with st.container(border=True):
            ...
            new_name = st.text_input(
                "Enter your name",
                placeholder="E.g. Mayank Chaudhary"
            )
            ...
            try:
                audio_data = st.audio_input(
                    'Record a short phrase like "I am Present, My name is Mayank."'
                )
            except Exception:
